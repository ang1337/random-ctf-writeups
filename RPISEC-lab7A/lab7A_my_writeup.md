...
```
The heap is used for storing OTP-encrypted messages. As you can see, there is no obvious memory corruption (at the first glance ;)), no UAF/double-free vulnerability as well. Let's take a look at this line of code resides in ```create_message``` function:
...
It is known that by default the division operation in C (and the vast majority of other programming languages) rounds down the result. Let's take a look at the ```struct msg```:
...
129/4 = 32 as well, because as I already mentioned, the result is rounded down. 130 and 131 emit 'false' in if statement as well.
There is ```read(0, &new_msg->message, new_msg->msg_len);``` right after this if statement. So I can write up to 131 bytes inclusively to the ```message``` array, which gives me an overflow of up to 3 bytes!
Sweet, so I can freely corrupt the first 3 bytes of ```msg_len``` struct member. 

After corrupting the current chunk ```msg_len``` value I can overflow the current chunk and overwrite the next chunk on the heap. What it can give me? Let's take a look:
...
The marked areas are related to the first chunk, malloc returned ```0x8a309d0```.
...
Now, I need to know where to place my payload in memory, but first of all let's check the exploit mitigations applied for this binary:
...
So I decided to craft a payload as a ROP chain. As you can see, I can write a lot of data to the heap, but I have very low interaction with the stack, so it seems that a ROP chain must be written to the heap and the stack pointer needs to be redirected to the heap somehow in order to execute the ROP chain from well controlled memory area. I can call an arbitrary function (in fact, jump to arbitrary address pushing ```messages[i]``` to the stack right before that, according to Linux x86 calling conventions). Let's see the program state right before the crash:

![state_before_crash](https://user-images.githubusercontent.com/45107680/95119263-5e8edb80-0754-11eb-85f4-b54b841bb546.png)

Interesting, ```edx``` contains some heap address - in fact, this is ```messages[i]``` address. It would be awesome if the binary contains a ROP gadget that allows to assign the ```edx``` register value to the stack pointer in order to pivot to the heap where the main payload can be easily written. It may be ```mov esp, edx ; ret``` or ```push edx ; pop esp ; ret``` or ```pop whatever ; pop esp ; ret``` gadgets. Unfortunately, the are no such gadgets in the binary :( It looks pretty obvious that somehow I need to pivot into the heap anyway, because this is the only memory area where I can freely write a lot of arbitrary data. Let's take a look at ```print_index``` function again:
```
int print_index()
{
    char numbuf[32];
...
Good news! I control 30 bytes in ```numbuf``` buffer passing the index validation check. Of course, it's too small space for the full payload, but it can be used to place a gadget address there that will allow me to redirect the stack pointer into the heap. So, I need a gadget like ```add esp, X ; ret``` in order to pivot into the ```numbuf```. It turned out that there is a bunch of such gadgets, but I needed to save as much stack space as possible, ```numbuf``` is not that big and I don't know yet if there are comfy gadgets to pivot easily to the heap from there. I've found a gadget that looks pretty neat to pivot into the controlled buffer on the stack - ```add esp, 0x20 ; mov eax, esi ; pop ebx ; pop esi ; ret```. Let's overwrite the second chunk function pointer with this gadget address and see the stack content during execution flow hijacking:
...
Green arrow - as I have 2 dummy pops from the stack pivot gadget, this memory can be used for overwriting with a gadget address that redirects the stack pointer to the heap
...
After pivoting into ```numbuf```, ```esp``` points to ```0xffa9d02c``` (marked with a red arrow on the previous screenshot), then there is an assignment of ```esi``` into ```eax``` (don't care), 2 pops (don't care, I'm not interested in ```ebx``` and ```esi``` register values for now, so these 7 bytes after the stack pivoting can be filled with garbage) and ret, which allows me to jump into the next gadget as I still have some controlled space in ```numbuf```. So let's jump into the above mentioned ```mov eax, edx ; ret```. 

```edx``` register contains a heap address which is assigned to ```eax```. 

```xchg eax, esp ; ret``` swaps ```esp``` and ```eax``` values between these registers. Thus, ```esp``` gets the ```edx``` value through ```eax```, which effectively pivots the stack pointer to the heap! So the payload for the ```numbuf``` is 1 (chunk index where the function pointer is overwritten by stack pivoting gadget) + 7 garbage bytes (padding the "don't care" memory space with garbage) + mov_eax_edx gadget address + xchg_eax_esp gadget address. The payload size fits nicely into the ```numbuf``` space constraints, lefting 12 remaining controlled bytes unused :)
...
That's how the main ROP chain looks in memory after redirecting a stack pointer to the heap:

![main_chain_layout](https://user-images.githubusercontent.com/45107680/95231835-598d6300-080c-11eb-9181-be94719dfdcf.png)

It computationally equals to ```execve("/bin/sh", NULL, NULL)```. A moment of truth:
...
By the way, I've searched for other solutions after I've solved the challenge and I've seen another solution that looks pretty interesting - the heap pointer was leaked from the stack via crafted format string and recursive call to ```print_index``` prior to ```printf``` due to stack layout constraints. Very smart and not obvious way to solve this challenge via some kind of stack grooming + info leak that is possible due to arbitrary call primitive. Also, in this solution a ROP chain was used to make a virtual memory page that belongs to heap executable via ```mprotect``` syscall, so the main payload is a shellcode, not ROP chain like in my solution. For all those who curious to check it out -> https://hackingiscool.pl/heap-overflow-with-stack-pivoting-format-string-leaking-first-stage-rop-ing-to-shellcode-after-making-it-executable-on-the-heap-on-a-statically-linked-binary-mbe-lab7a/
